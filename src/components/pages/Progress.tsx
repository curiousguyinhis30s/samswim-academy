...
import { Trophy, Medal, SwimmerButterfly, WaterDrop } from '@/components/icons/SwimmingIcons'
...
// AnimatedCounter component for count-up effect
...
// PulsingProgressRing component with pulse animation on percentage change
function PulsingProgressRing({
  progress,
  size = 'sm',
  className = ''
}: {
  ...
  useEffect(() => {
    if (!ringRef.current || prevProgress.current === progress) return
  ...
  const strokeColor = progress >= 75 ? '#84cc16' : progress >= 50 ? '#14b8a6' : '#0ea5e9'
  const circumference = size === 'lg' ? 352 : 290
  const radius = size === 'lg' ? '56' : '46%'
  const cx = size === 'lg' ? '64' : '50%'
  const cy = size === 'lg' ? '64' : '50%'
  const strokeWidth = size === 'lg' ? 8 : 4

  return (
    <svg className={`absolute inset-0 w-full h-full -rotate-90 ${className}`}>
      <circle
        cx={cx}
        cy={cy}
        r={radius}
        fill="none"
        stroke={size === 'lg' ? 'rgba(255,255,255,0.2)' : '#e0f2fe'}
  ...
  return (
    <div className="min-h-screen ocean-bg">
      <div className="px-4 sm:px-8 py-6 max-w-7xl mx-auto">
          ...
          <div className="mb-8">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-tennis-400 to-tennis-600 flex items-center justify-center shadow-lg">
        ...
        {/* Student Cards Grid with StaggerChildren animation */}
        <FadeIn delay={0.2}>
          <div className="mb-8">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="relative mx-auto w-20 h-20 sm:w-24 sm:h-24 mb-3">
                      ...
                      <PulsingProgressRing progress={progress} size="sm" />
                      {/* Progress Badge */}
        ...
        {!selectedClient ? (
          <FadeIn delay={0.3}>
            <div className="bg-white rounded-3xl border border-ocean-100 p-12 text-center">
              <div className="w-20 h-20 mx-auto mb-6 bg-gradient-to-br from-ocean-100 to-sea-100 rounded-3xl flex items-center justify-center">
                <SwimmerButterfly size={40} className="text-ocean-400" />
              </div>
          ...
          <>
            {/* Selected Student Hero Card with ScaleIn animation */}
                ...
                <div className="relative flex flex-col sm:flex-row items-center gap-6">
                  <div className="relative">
                    ...
                    <div className="absolute -bottom-2 -right-2 w-10 h-10 bg-tennis-500 rounded-2xl flex items-center justify-center text-sm font-bold shadow-lg">
                      <AnimatedCounter value={overallStats.percentage} duration={1} />%
                    </div>
                  </div>
                  ...
                  <div className="flex-1 text-center sm:text-left">
                    ...
                    <div className="flex flex-wrap justify-center sm:justify-start gap-3">
                      <div className="px-4 py-2 bg-white/10 backdrop-blur rounded-xl">
                        <div className="flex items-center gap-2">
                          <Medal size={16} className="text-tennis-400" />
                          <p className="text-2xl font-bold"><AnimatedCounter value={overallStats.mastered} duration={1.2} /></p>
                        </div>
                        <p className="text-xs text-ocean-100">Mastered</p>
                      </div>
                      <div className="px-4 py-2 bg-white/10 backdrop-blur rounded-xl">
                        <p className="text-2xl font-bold"><AnimatedCounter value={overallStats.inProgress} duration={1.2} /></p>
                        <p className="text-xs text-ocean-100">In Progress</p>
                      </div>
                      <div className="px-4 py-2 bg-white/10 backdrop-blur rounded-xl">
                        <p className="text-2xl font-bold"><AnimatedCounter value={overallStats.total - overallStats.mastered - overallStats.inProgress} duration={1.2} /></p>
                  ...
                  <div className="hidden lg:block">
                    ...
                    <div className="relative w-32 h-32">
                      <PulsingProgressRing progress={overallStats.percentage} size="lg" />
                      <div className="absolute inset-0 flex flex-col items-center justify-center">
                        <p className="text-3xl font-bold"><AnimatedCounter value={overallStats.percentage} duration={1.5} />%</p>
            ...
            <div className="space-y-6">
              {skillCategories.map((category, categoryIndex) => {
                ...
                return (
                  <FadeIn key={category.id} delay={0.1 * categoryIndex}>
                    <div className="bg-white rounded-3xl border border-ocean-100 overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                        ...
                        <div className="flex items-center justify-between mb-4">
                          ...
                          <div className="text-right">
                            <p className="text-3xl font-bold text-ocean-900">
                              <AnimatedCounter value={progress} duration={1} />%
                            </p>
                          ...
                          {categorySkills.map(skill => {
                            ...
                            return (
                              <button
                                ...
                                className="group flex items-center gap-4 p-4 bg-gradient-to-br from-ocean-50/50 to-sea-50/50 rounded-2xl hover:from-ocean-50 hover:to-sea-50 border border-ocean-100/50 hover:border-ocean-200 transition-all text-left hover:scale-[1.02] hover:shadow-md"
                              >
                                <div className={`w-12 h-12 rounded-2xl ${levelInfo.color} flex items-center justify-center text-white font-bold text-lg shadow-sm transition-transform group-hover:scale-110`}>
                                  {level}
                                </div>
                                <div className="flex-1 min-w-0">
                                  <p className="font-semibold text-ocean-900 truncate">{skill.name}</p>
                                  <p className={`text-sm font-medium ${levelInfo.textColor} transition-colors group-hover:opacity-80`}>{levelInfo.label}</p>
                                </div>
            ...
            {/* Skill Level Legend with hover effects */}
            <FadeIn delay={0.4}>
              <div className="mt-8 bg-white rounded-3xl border border-ocean-100 p-6 shadow-sm">
                ...
                <div className="flex flex-wrap gap-3">
                  {SKILL_LEVELS.map(level => (
                    <div
                      key={level.value}
                      className={`flex items-center gap-2 px-4 py-2 rounded-2xl ${level.ring} ring-2 bg-white transition-all duration-200 hover:scale-105 hover:shadow-md cursor-default`}
                    >
                      <div className={`w-8 h-8 rounded-xl ${level.color} flex items-center justify-center text-white font-bold text-sm shadow-sm transition-transform hover:scale-110`}>
                        {level.value}
                      </div>
              ...
              <div className="grid grid-cols-5 gap-2">
                {SKILL_LEVELS.map(level => (
                  <button
                    ...
                    className={`flex flex-col items-center gap-2 p-3 rounded-2xl border-2 transition-all duration-200 hover:scale-105 ${
                      newLevel === level.value
                        ? 'border-ocean-500 bg-ocean-50 shadow-ocean'
                        : 'border-ocean-100 hover:border-ocean-200 bg-white'
                    }`}
                  >
                    <div className={`w-10 h-10 rounded-xl ${level.color} flex items-center justify-center text-white font-bold shadow-sm transition-transform ${newLevel === level.value ? 'scale-110' : ''}`}>
                      {level.value}
                    </div>
